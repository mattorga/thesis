...
const centerAnimationCheckbox = document.getElementById('centerAnimationCheckbox');
...
loader.load(
  'http://localhost:8000/davidpagnon%20Pose2Sim_Blender%20main%20Examples/untitled.fbx',
  (object) => {
...
forwardBtn.addEventListener('click', () => {
  if (mixer) {
    ...
    console.log(`[Forward] Current mixer.time: ${mixer.time}, timeScale: ${timeScaleBuffer}`);
    if (isPlaying) {
...
function onClick(event) {
  ...
  if (animatedObject) {
    ...
    if (intersects.length > 0) {
      ...
      if (highlightedBone) {
        highlightedBone.material = originalMaterials.get(highlightedBone) || highlightedBone.material;
      }
